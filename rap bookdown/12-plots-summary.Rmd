...
This page will take you through the steps for the data analysis we wish to do. It will comprise of manipulating and plotting data and generating statistics. It will go through the following steps:

- Read in the data from the previous chapter
- Filter the dataset
...
print(unique(data$UnofficialStandardIndustrialClassification))
```
...
minimum <- min(data$v4_1)
maximum <- max(data$v4_1)
average <- mean(data$v4_1)
```
